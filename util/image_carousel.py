...
class ImageCarousel(QWidget):
    def __init__(self, guest_id=None):
        ...
        self.temporary_images = []  #   Store images before guest is created
        ...
        if self.guest_id:
            self.load_guest_images()

    def load_guest_images(self):
        """ Load guest images from the database and keep the delete button in the top-right corner """
        if not self.guest_id:
            return

        self.current_index = 0  #   Reset image index when switching guests

        conn = sqlite3.connect("hotel_management.db")
        cursor = conn.cursor()
        cursor.execute("SELECT id, image_path FROM GuestImages WHERE guest_id=?", (self.guest_id,))
        images = cursor.fetchall()
        conn.close()

        self.image_paths = [(img[0], os.path.join(GUEST_IMAGE_DIR, img[1])) for img in images]
        ...
        for image_id, image_path in self.image_paths:
            if os.path.exists(image_path):
                ...
                """)
                delete_button.clicked.connect(lambda _, img_id=image_id: self.delete_guest_image(img_id))  #   Delete on click
        ...
        self.update_image_counter()  #   Update counter when switching guests
    ...
    def delete_guest_image(self, image_id):
        """ Delete guest image from database and refresh the UI. """
        ...
        if confirm == QMessageBox.StandardButton.Yes:
            ...
            if image:
                image_path = os.path.join(GUEST_IMAGE_DIR, image[0])
                if os.path.exists(image_path):
            ...
            self.load_guest_images()  #   Corrected: Call `load_guest_images()` directly
    ...
    def remove_temporary_image(self, file_path):
        """ Remove a temporary image before adding the guest. """
    ...
    def add_image_to_carousel(self, file_path, is_temporary=False):
        """ Add an image to the carousel (used when adding a new guest before saving). """
        if is_temporary:
